...
- **세션 기억** — 화면 공유를 새로 켜기 전까지 이번 세션에서 푼 문제를 모두 기억합니다. "지금까지 푼 것 정답 다 알려줘" 처럼 물으면 정리해서 답합니다. 지난 문제의 이미지까지 매번 다시 보내지는 않고 `문제/정답` 요약만 문맥으로 넣어, 비용과 지연이 불어나지 않게 했습니다.
- **새로 시작** — 풀이 패널의 `🆕 새로 시작`(기억 중인 문제 수가 함께 표시됩니다) 또는 화면 공유·카메라를 새로 켜면 이전 대화와 기록을 모두 지우고 처음부터 시작합니다.
...
js/store.js             프로바이더 정의 및 설정/키 저장 (앱은 Keystore, 웹은 localStorage)
...
**AI 풀이에는 인터넷이 반드시 필요합니다.** 화면을 읽는 것은 기기에서 하지만, 푸는 것은 선택한 AI 서비스의 API 라서 완전 오프라인 풀이는 불가능합니다.

- **와이파이가 없어도 모바일 데이터로 동작합니다.** 앱에 와이파이 전용 제한은 없습니다.
- 연결이 끊기면 앱이 종료되지 않고 `다시 시도` 버튼과 함께 이유를 알려 줍니다. **화면 공유는 그대로 유지**되므로 다시 캡처할 필요가 없습니다.
- 연결이 돌아오면 실패했던 분석을 **자동으로 이어서** 시도합니다.
- 연결이 없는 동안에는 자동 감지가 요청을 보내지 않습니다(헛돌지 않게).
- 모바일 데이터에서만 안 될 때는 대부분 **데이터 절약(Data Saver)** 때문입니다. 앱이 이를 감지해 *설정 → 네트워크 → 데이터 절약*에서 허용하라고 안내합니다.
- 앱 화면 자체는 오프라인에서도 열립니다(웹은 서비스 워커, 앱은 내장 자산).
...
- ⚠️ **키가 환경변수에 있다고 해서 자동으로 켜지지 않습니다.** 사이트가 공개되어 있으면 주소를 아는 누구나 사이트 주인의 크레딧을 쓰게 되므로, **기본값은 꺼짐**입니다. 주인이 다음 중 하나를 명시적으로 설정해야 켜집니다.
  - `PROXY_ENABLED=true` — 누구나 서버 키로 사용 (비용은 사이트 주인 부담)
  - `PROXY_ACCESS_CODE=<코드>` — 코드를 아는 사람만 사용. 설정 화면에 코드 입력란이 나타나며, 요청 시 `x-screensolver-code` 헤더로 전달됩니다.
- 보조 장치로 IP 당 10분 20회 제한이 있습니다 (`PROXY_RATE_LIMIT`, `0` 이면 해제). 다만 **이것만 믿으면 안 됩니다** — 카운터가 함수 인스턴스 메모리에만 있어 콜드 스타트·다중 인스턴스에서 초기화되고, 출발 IP 를 바꾸면 우회됩니다. 실제 배포본에서 25회를 연속 호출해 보았지만 제한이 걸리지 않는 것을 확인했습니다. 비용을 확실히 막는 방법은 프록시를 켜지 않는 것입니다.
...
- **Android 앱**: 화면 공유 중에는 화면에 보이는 내용이 선택한 AI 로 전송될 수 있습니다. 시작·중지는 항상 사용자가 직접 하며, 공유 중에는 알림이 상시 표시됩니다. 캡처 이미지는 앱의 서버로 저장되지 않습니다. 기록은 이 기기에만 남으며, **원본 캡처 이미지는 저장하지 않고** 목록용 축소본과 풀이 내용만 보관합니다. `🆕 새로 시작`을 누르면 저장된 기록까지 모두 지워집니다.
- **API 키 보관**: 앱에서는 Android Keystore 안의 키로 AES-GCM 암호화해 저장합니다. 암호화 키 자체는 하드웨어에 남아 앱이 읽어낼 수 없으므로, 저장 파일만 빼내도 값을 복원할 수 없습니다. 예전 버전이 평문으로 남긴 키는 앱을 켜면 자동으로 옮겨지고 평문 쪽은 지워집니다. 웹 브라우저에는 이런 저장소가 없어 `localStorage` 평문이며, 설정 화면에 어느 쪽인지 그대로 표시됩니다. 공용 기기에서는 사용 후 🗑 로 키를 지우세요.
- 클라우드 백업·기기 간 전송에서 앱 데이터를 제외해(`allowBackup=false` + `dataExtractionRules`) 키가 기기 밖으로 나가지 않습니다.
