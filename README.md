...
- ⚠️ **서버 프록시를 켜면 비용이 사이트 주인에게 청구됩니다.** 사이트가 공개되어 있으면 주소를 아는 누구나 그 키를 쓸 수 있습니다. 다음 장치를 두었지만 한계가 있습니다.
  - IP 당 10분에 20회 제한 (`PROXY_RATE_LIMIT` 으로 조정, `0` 이면 해제). 함수 인스턴스 메모리에만 기록하므로 콜드 스타트·다중 인스턴스에서는 초기화됩니다. 실수로 인한 폭주는 막지만 작정한 공격은 막지 못합니다.
  - `PROXY_ACCESS_CODE` 를 설정하면 그 값을 `x-screensolver-code` 헤더로 보내는 요청만 서버 키를 쓸 수 있습니다.
  - 확실히 막으려면 **환경변수를 지우세요.** 그러면 각 사용자가 자기 키를 입력하는 방식으로 동작합니다.
