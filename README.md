...
모바일 브라우저는 화면 공유(`getDisplayMedia`)를 지원하지 않으므로, 폰에서 접속하면 **카메라 탭이 자동으로 선택**되고 화면 공유 탭은 비활성화됩니다.
...
| iOS Safari · Chrome (iOS 14.3+) | ❌ | ✅ | ✅ |

지원하지 않는 소스의 탭은 자동으로 비활성화됩니다. 카메라 사용에는 HTTPS와 카메라 권한 허용이 필요합니다.
