...
- **빈칸 채우기 지원** — 밑줄 `___`, 괄호 `( )`, 네모 `□`, `(A)/(B)`, `(가)/(나)`, `ⓐ/ⓑ` 등을 빈칸으로 인식해 들어갈 말을 답합니다. 빈칸이 여러 개면 순서대로 답하고, **빈칸을 채운 최종 문장**까지 함께 보여 줘 맞는지 바로 확인할 수 있습니다.
- **어떤 문제를 읽었는지 표시** — 답변 맨 위 「읽은 문제」로 무엇을 풀었는지 먼저 보여 줍니다. 화면에 문제가 여러 개면 가운데에 온전히 보이는 문제를 고르며, 엉뚱한 문제를 잡았으면 `영역 지정`으로 짚어 다시 풀 수 있습니다.
